...
function renderResults(): void {
  ...
  const review = results.perQuestion.map((item, index) => {
    ...
    return `<details class="review-item"><summary><span>${index + 1}. ${clean(item.question.skill)}</span><b class="${item.correct ? 'right' : item.answered ? 'wrong' : 'muted'}">${status}</b></summary><div class="review-content">${questionText}${assets}${choices ? `<div class="review-choices"><strong>Answer choices</strong>${choices}</div>` : ''}<p>Your answer: ${clean(item.response || '—')} · Correct answer: ${clean(item.question.answer)}</p><p>${clean(item.question.explanation)}</p><div class="review-actions"><button class="quiet" data-review-index="${questionIndex}" ${questionIndex < 0 ? 'disabled' : ''}>Go to question</button></div></div></details>`;
  }).join('');
